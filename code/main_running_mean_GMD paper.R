# UPDATED MAIN SCRIPT FOR THE RUNNING MEAN METHOD
rm(list = ls())
cat('\014')
...
source ('./R/fileOr2pModelPlus.R')      # imports input dataset files (MODIS fAPAR & fluxnet)                          (updated function)
source ('./R/createInputPmodelPlus.R')  # creates the input dataset by merging the fluxnet2015 and MODIS fAPAR dataset
source ('./R/pmodelPlus.R')             # applies the P model according to the settings                                (updated function)
...
# Settings files:
# file_settaggio = 'C:/pModelPlus/drylands/FLUXNET2015_Feb2020/FLUXNET_sites/SETTINGS/AU-Wom_test_3a2010-2012.csv'    # 'MAX: noon approach'  
# file_settaggio = 'C:/pModelPlus/drylands/FLUXNET2015_Feb2020/FLUXNET_sites/SETTINGS/AU-GWW_test_3a2013-2014.csv'    # 'MAX: noon approach'
...
# dataLAI = read.csv('C:/Users/giuli/Desktop/fluxnet_sites/dataAUWom_LAI.csv')
# data$LAI = dataLAI$LAI
...
if (df_settaggio$xi_acclimated == 'off') {
  ...
  if (is.na(df_settaggio$phi0_method) ) {
    ...
    phi0 =(1/8) *(0.352+0.022*dfReplicate[,'TempOpt'] -
                    0.00034*dfReplicate[,'TempOpt']^(2) )      # Temperature dependence function of phi0 (Bernacchi et al.,2003)
  } else {
    phi0 = phi0Method
  }
  
  pos_neg = which(phi0 < 0)                       # never negative      
  if (length (pos_neg)> 0 ) phi0[pos_neg] = 0
  rm (pos_neg)
...
dataOr[,'ci1'] = ( dataOr[,'xiPa'] * dataOr[,'ca'] +
                     dataOr[,'GammaStarM']*sqrt(dataOr[,'VPDPa']))/(
                       dataOr[,'xiPa']+sqrt(dataOr[,'VPDPa']))

# CALCULATE the assimilation rate: Ac
# acclimated Ac with the acclimated xiPa term
Ac = dataOr[,'vcmaxAdjusted']*(
  dataOr[,'ci1'] - dataOr[,'GammaStarM']) /
  (dataOr[,'ci1'] + dataOr[,'kmPa'])                                             #[micromol/m2s]

# CALCULATE the assimilation rate: AJ
# acclimated AJ with the acclimated xiPa term
AJ = (dataOr[,'J']/4)*(dataOr[,'ci1'] -
                         dataOr[,'GammaStarM'])/(dataOr[,'ci1'] + 2*dataOr[,'GammaStarM'])    #[micromol/m2s]
...
cat(sprintf('Calculate GPPpOpt1, please wait ....\n'))
for ( cy1 in seq(1,nrow(dataOr)) ) {
  tmp = c(dataOr[,'Ac1Opt1'][cy1],dataOr[,'AJp1Opt1' ][cy1])
  posNa = which(is.na(tmp) == 1)
  if ( length(posNa) > 0 ) tmp = tmp[-1*posNa]
  rm(posNa)
  if (length(tmp) == 2 )
    dataOr[,'GPPpOpt1'][cy1] = min(tmp)
}
rm(cy1,tmp)

cat(sprintf('Calculate GPPpOpt1 .... OK\n'))
